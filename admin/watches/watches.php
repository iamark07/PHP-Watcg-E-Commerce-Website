...
<body>
            ...
            <nav class="hidden lg:block">
                <ul class="animate_text_color flex gap-5">
                    ...
                    ?>
                        ...
                        <li class="text-blue-500"><a href="watches.php">Men</a></li>
    ...
    <main class="bg-zinc-300 w-full min-h-[100dvh] flex flex-col items-center gap-10 py-20 px-5 md:px-5">
        ...
        <div class="relative w-full">
            ...
            $offset = ($page - 1) * $limit;
            $sql = "SELECT watches.watch_id, watches.watch_title, watches.watch_description, watches.category, watches.watch_date, all_watches.save_watch_id, all_watches.target_audience, watch_target_audience.target_audience_id, category.category_id, category.category_product_name, watch_brand.brand_id, watch_brand.watch_brand_name, user.username, watches.watch_amount FROM watches
            LEFT JOIN all_watches ON all_watches.save_watch_id = watches.watch_id
            LEFT JOIN watch_target_audience ON watch_target_audience.target_audience_id = all_watches.target_audience
            LEFT JOIN category ON watches.category = category.category_id
            LEFT JOIN watch_brand ON watches.watch_brand_name = watch_brand.brand_id
            LEFT JOIN user ON watches.Author = user.user_id
            WHERE all_watches.target_audience = 1
            ORDER BY watches.watch_id DESC LIMIT {$offset},{$limit}";
