...
$sql = "INSERT INTO watches (watch_title, watch_description, watch_brand_name, category, watch_date, Author, watch_img, watch_amount, img_folder_name)
        VALUES ('{$title}', '{$description}', {$brand}, {$category}, '{$date}', {$author}, '{$new_file_name}', '{$amount}', '{$clean_title}');";

// Execute the main insert query for watches
if (mysqli_query($conn, $sql)) {
    ...
    $source_table = 'watches'; // Example for wall watches. Adjust based on the actual source.
    $all_brand_id = 1;
    $sql_all_watches = "
