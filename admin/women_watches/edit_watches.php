...
<body>
            ...
            <nav class="hidden lg:block">
                <ul class="animate_text_color flex gap-5">
                    ...
                    if ($_SESSION["user_role"] == 1) {
                        ...
                        <li class="text-blue-500"><a href="watches.php">Women</a></li>
    ...
    <main class="flex justify-center items-center flex-col bg-zinc-100 p-5 md:px-20">
        ...
        $sql = "SELECT women_watches.watch_id, women_watches.watch_title, women_watches.watch_description, women_watches.category AS watches_category, women_watches.watch_img, 
        women_watches.watch_brand_name, women_watches.watch_amount, women_watches.img_folder_name, all_watches.save_watch_id, all_watches.target_audience, all_watches.category AS all_category,
        watch_target_audience.target_audience_name, category.category_id, category.category_product_name, user.username 
        FROM women_watches
        LEFT JOIN all_watches ON all_watches.save_watch_id = women_watches.watch_id
        LEFT JOIN watch_target_audience ON all_watches.target_audience = watch_target_audience.target_audience_id
        LEFT JOIN category ON women_watches.category = category.category_id
        LEFT JOIN watch_brand ON women_watches.watch_brand_name = watch_brand.brand_id
        LEFT JOIN user ON women_watches.Author = user.user_id
        WHERE women_watches.watch_id = {$watches_id} AND target_audience = {$watch_cat}";
