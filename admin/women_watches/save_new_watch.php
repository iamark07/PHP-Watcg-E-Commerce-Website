...
$sql = "INSERT INTO women_watches (watch_title, watch_description, watch_brand_name, category, watch_date, Author, watch_img, watch_amount, img_folder_name)
        VALUES ('{$title}', '{$description}', {$brand}, {$category}, '{$date}', {$author}, '{$new_file_name}', '{$amount}', '{$clean_title}');";

// Get the last inserted watch ID
if (mysqli_query($conn, $sql)) {
    ...
    $source_table = 'women_watches'; // Example for wall watches. Adjust based on the actual source.
    $all_brand_id = 1;
    $sql_all_watches = "
    ...
    VALUES ({$watch_id}, '{$source_table}', '{$title}', '{$description}', {$brand}, '{$target_audience}', {$category}, '{$date}', {$author}, '{$new_file_name}', '{$amount}', '{$clean_title}' ,{$all_brand_id});
        ...
        UPDATE watch_brand SET count_brand_watch = count_brand_watch + 1 WHERE brand_id = {$brand};
        UPDATE watch_target_audience SET count_target_audience = count_target_audience + 1 WHERE target_audience_id = {$target_audience};
    ";

    // Execute the all_watches insert and updates
    if (mysqli_multi_query($conn, $sql_all_watches)) {
        ...
        echo '<div class="text-red-500">Query for all_watches Failed</div>';
    }
} else {
