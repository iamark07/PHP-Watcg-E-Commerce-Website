...
<body>
    ...
    <main class="bg-zinc-300 w-full h-[100dvh] overflow-auto flex flex-col items-center gap-10 py-20 px-5 md:p-20">
            ...
            ?>
                ...
                <div class="table_overflow overflow-x-auto w-full">
                    <table class="w-full bg-zinc-800 mt-10">
                        <thead>
                            <tr>
                                <td class="text-white font-semibold text-lg border border-zinc-600 text-center text-nowrap p-5">Sl No</td>
                                <td class="text-white font-semibold text-lg border border-zinc-600 text-center text-nowrap p-5">Full Name</td>
                                <td class="text-white font-semibold text-lg border border-zinc-600 text-center text-nowrap p-5">Username</td>
                                <td class="text-white font-semibold text-lg border border-zinc-600 text-center text-nowrap p-5">Roll</td>
                        ...
                        <tbody class="bg-zinc-200">
                            ...
                            ?>
                                <tr>
                                    <td class="text-black text-md border border-zinc-500 text-center text-nowrap p-5"><?php echo $row['user_id']; ?></td>
                                    <td class="text-black text-md border border-zinc-500 text-center text-nowrap p-5"><?php echo $row['first_name'] . " " . $row['last_name']; ?></td>
                                    <td class="text-black text-md border border-zinc-500 text-center text-nowrap p-5"><?php echo $row['username']; ?></td>
                                    <td class="text-black text-md border border-zinc-500 text-center text-nowrap p-5">
                                        <?php
                                        if ($row['role'] == 1) {
                                            echo "Admin";
                                        } else {
                                    ...
                                    <?php
                                    // Check if the current session's user is an admin and is viewing their own record
                                    if ($_SESSION["user_role"] == 1 && $_SESSION["user_id"] == $row['user_id']) {
                                        echo '  
                                            <td class="text-black font-semibold text-lg border border-zinc-500 text-center p-5">
                                                <a href="edit_user.php?id=' . $row["user_id"] . '"><i class="ri-pencil-line"></i></a>
                                            </td>
                                            <td class="text-black font-semibold text-lg border border-zinc-500 text-center p-5">
                                                <a href="delete_user.php?id=' . $row["user_id"] . '"><i class="ri-delete-bin-2-line"></i></a>
                                            </td>';
                                    } else {
                                        // For other cases (e.g. admin viewing other users' rows), don't show edit/delete buttons
                                        echo '  
                                            <td class="text-black font-semibold text-lg border border-zinc-500 text-center p-5"></td>
                                            <td class="text-black font-semibold text-lg border border-zinc-500 text-center p-5"></td>';
                                    }
                                    ?>

                                    <!-- <td class="text-black font-semibold text-lg border border-zinc-500 text-center p-5">
                                    <a href="edit_user.php?id=<?php echo $row['user_id']; ?>"><i class="ri-pencil-line"></i></a>
                                </td>
                                <td class="text-black font-semibold text-lg border border-zinc-500 text-center p-5">
                                    <a href="delete_user.php?id=<?php echo $row['user_id']; ?>"><i class="ri-delete-bin-2-line"></i></a>
                                </td> -->
                                </tr>
                            <?php
                            }
                ...
                <button class="bg-blue-400 text-white text-lg px-5 py-3 absolute right-0 -top-10"><a href="../register.php">Add User</a></button>
            <?php
            }
        ...
        <ul class="flex justify-center gap-3">';
                ...
                if ($page > 1) {
                    echo '<li><a href="users.php?page=' . ($page - 1) . '" class="text-white bg-blue-400 px-4 py-3 rounded">Prev</a></li>';
                }

                // Generate page number links
                for ($i = $start_page; $i <= $end_page; $i++) {
                    $active = $i == $page ? '!bg-blue-500' : '';
                    echo '<li><a href="users.php?page=' . $i . '" class="text-white ' . $active . ' bg-blue-400 px-4 py-3 rounded">' . $i . '</a></li>';
                }

                if ($page < $total_pages) {
                    echo '<li><a href="users.php?page=' . ($page + 1) . '" class="text-white bg-blue-400 px-4 py-3 rounded">Next</a></li>';
                }
