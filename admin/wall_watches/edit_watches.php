...
<body>
            ...
            <nav class="hidden lg:block">
                <ul class="animate_text_color flex gap-5">
                    ...
                    <?php if ($_SESSION["user_role"] == 1) { ?>
                        <li class="text-blue-500"><a href="watches.php">Wall Clock</a></li>
    ...
    <main class="flex justify-center items-center flex-col bg-zinc-100 p-5 md:px-20">
        ...
        $sql = "SELECT wall_watches.watch_id, wall_watches.watch_title, wall_watches.watch_description, wall_watches.category AS watches_category, 
        wall_watches.watch_img, wall_watches.watch_brand_name, wall_watches.watch_amount, wall_watches.img_folder_name, 
        ...
        FROM wall_watches
        LEFT JOIN all_watches ON all_watches.save_watch_id = wall_watches.watch_id
        LEFT JOIN watch_target_audience ON watch_target_audience.target_audience_id = all_watches.target_audience
        LEFT JOIN category ON wall_watches.category = category.category_id
        LEFT JOIN watch_brand ON wall_watches.watch_brand_name = watch_brand.brand_id
        LEFT JOIN user ON wall_watches.Author = user.user_id
        WHERE wall_watches.watch_id = {$watches_id} AND target_audience = {$watch_cat}";
    ...
    <script>
        function validateForm() {
            ...
            if (watchDesc.length < 5) {
                document.getElementById('watch_desc_error').innerText = 'Watch description must be at least 5 characters.';
                valid = false;
            }
