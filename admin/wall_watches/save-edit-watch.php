...
// Fetch img_folder_name and current image extension from both wall_watches and all_watches tables
$folder_query = "SELECT img_folder_name, watch_img FROM wall_watches WHERE watch_id = $watch_id
                UNION ALL
...
// Update the watch details in the wall_watches table
$sql = "UPDATE wall_watches SET 
...
$sql .= " UPDATE all_watches SET  
            ...
            watch_img = '$full_image_name',
            img_folder_name = '$img_folder_name',
            source_table = 'wall_clock',
            all_brand_id = 2
        WHERE save_watch_id = $all_watch_id AND target_audience = {$watch_cat};";

// Check if brand has changed
if ($brand !== $original_brand) {
    $sql .= " UPDATE clock_brand SET count_brand_watch = count_brand_watch + 1 WHERE brand_id = $brand;";
    $sql .= " UPDATE clock_brand SET count_brand_watch = count_brand_watch - 1 WHERE brand_id = $original_brand;";
}
